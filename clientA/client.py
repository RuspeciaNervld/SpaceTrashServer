...
from config import CLIENT_A_WATCH_PATH as WATCH_PATH, CLIENT_A_SERVER_URL as SERVER_URL, CLIENT_A_CHECK_INTERVAL as CHECK_INTERVAL, CLIENT_A_TARGET_FILENAME as TARGET_FILENAME
...
class FileHandler(FileSystemEventHandler):
    ...
    def on_modified(self, event):
        ...
        if not event.is_directory and os.path.basename(event.src_path) == TARGET_FILENAME:
            ...
            # 避免短时间内重复上传（5秒内只上传一次）
            if current_time - self.last_upload_time > 5:
...
def check_existing_file():
    ...
    检查目标文件是否已存在
    """
    target_path = os.path.join(WATCH_PATH, TARGET_FILENAME)
